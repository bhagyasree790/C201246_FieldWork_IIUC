...
def get_or_create_publisher(db: Session, name: str, email: str):
    publisher = db.query(models.Publisher).filter(models.Publisher.name == name).first()
    if publisher is None:
        publisher = models.Publisher(name=name, email=email)
        db.add(publisher)
        db.commit()
...
def create_news(db: Session, news: schemas.NewsCreate):
    ...
    publisher = get_or_create_publisher(db, news.news_publisher, f"{news.news_publisher}")
